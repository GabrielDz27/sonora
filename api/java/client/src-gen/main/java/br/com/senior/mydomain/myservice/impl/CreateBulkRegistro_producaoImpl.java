...
import br.com.senior.mydomain.myservice.CreateBulkRegistro_producaoInput;
import br.com.senior.mydomain.myservice.CreateBulkRegistro_producaoOutput;
 ...
 * 
 */
public class CreateBulkRegistro_producaoImpl {
	...
	public CreateBulkRegistro_producaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(CreateBulkRegistro_producaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.CREATE_BULK_REGISTROPRODUCAO, //
	...
	*/
	public CreateBulkRegistro_producaoOutput createBulkRegistro_producao(CreateBulkRegistro_producaoInput input, long timeout) {
		...
		CreateBulkRegistro_producaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), CreateBulkRegistro_producaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<CreateBulkRegistro_producaoOutput> createBulkRegistro_producaoRequest(CreateBulkRegistro_producaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkRegistro_producaoOutput.class);
	}
	...
	* The 'createBulk' request primitive for the Registro_producao entity.
	*/
	public CompletableFuture<CreateBulkRegistro_producaoOutput> createBulkRegistro_producaoRequest(CreateBulkRegistro_producaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkRegistro_producaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
