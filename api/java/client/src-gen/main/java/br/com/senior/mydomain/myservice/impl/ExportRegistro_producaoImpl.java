...
import br.com.senior.mydomain.myservice.ExportRegistro_producaoInput;
import br.com.senior.mydomain.myservice.ExportRegistro_producaoOutput;
 ...
 * 
 */
public class ExportRegistro_producaoImpl {
	...
	public ExportRegistro_producaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportRegistro_producaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.EXPORT_REGISTROPRODUCAO, //
	...
	*/
	public ExportRegistro_producaoOutput exportRegistro_producao(ExportRegistro_producaoInput input, long timeout) {
		...
		ExportRegistro_producaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportRegistro_producaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ExportRegistro_producaoOutput> exportRegistro_producaoRequest(ExportRegistro_producaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportRegistro_producaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ExportRegistro_producaoOutput> exportRegistro_producaoRequest(ExportRegistro_producaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportRegistro_producaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
