...
import br.com.senior.mydomain.myservice.FinalizarInput;
import br.com.senior.mydomain.myservice.FinalizarOutput;
...
public class FinalizarImpl {
	...
	public FinalizarImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FinalizarInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FINALIZAR, //
	...
	*/
	public FinalizarOutput finalizar(FinalizarInput input, long timeout) {
		...
		FinalizarOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FinalizarOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FinalizarOutput> finalizarRequest(FinalizarInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FinalizarOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<FinalizarOutput> finalizarRequest(FinalizarInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FinalizarOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
