...
import br.com.senior.mydomain.myservice.ImportPecaInput;
import br.com.senior.mydomain.myservice.ImportPecaOutput;
...
public class ImportPecaImpl {
	...
	public ImportPecaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportPecaInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_PECA, //
	...
	*/
	public ImportPecaOutput importPeca(ImportPecaInput input, long timeout) {
		...
		ImportPecaOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportPecaOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportPecaOutput> importPecaRequest(ImportPecaInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportPecaOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportPecaOutput> importPecaRequest(ImportPecaInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportPecaOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
