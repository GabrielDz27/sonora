...
import br.com.senior.mydomain.myservice.ImportRegistroProducaoInput;
import br.com.senior.mydomain.myservice.ImportRegistroProducaoOutput;
 ...
 * 
 */
public class ImportRegistroProducaoImpl {
	...
	public ImportRegistroProducaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportRegistroProducaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_REGISTRO_PRODUCAO, //
	...
	*/
	public ImportRegistroProducaoOutput importRegistroProducao(ImportRegistroProducaoInput input, long timeout) {
		...
		ImportRegistroProducaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportRegistroProducaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportRegistroProducaoOutput> importRegistroProducaoRequest(ImportRegistroProducaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportRegistroProducaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportRegistroProducaoOutput> importRegistroProducaoRequest(ImportRegistroProducaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportRegistroProducaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
