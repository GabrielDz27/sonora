...
import br.com.senior.mydomain.myservice.ImportRegistro_producaoInput;
import br.com.senior.mydomain.myservice.ImportRegistro_producaoOutput;
 ...
 * 
 */
public class ImportRegistro_producaoImpl {
	...
	public ImportRegistro_producaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportRegistro_producaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_REGISTROPRODUCAO, //
	...
	*/
	public ImportRegistro_producaoOutput importRegistro_producao(ImportRegistro_producaoInput input, long timeout) {
		...
		ImportRegistro_producaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportRegistro_producaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportRegistro_producaoOutput> importRegistro_producaoRequest(ImportRegistro_producaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportRegistro_producaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportRegistro_producaoOutput> importRegistro_producaoRequest(ImportRegistro_producaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportRegistro_producaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
