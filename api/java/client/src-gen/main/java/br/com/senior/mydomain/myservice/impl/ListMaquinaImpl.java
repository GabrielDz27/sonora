...
public class ListMaquinaImpl {
	...
	public ListMaquinaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Maquina.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.LIST_MAQUINA, //
	...
	*/
	public Maquina.PagedResults listMaquina(Maquina.PageRequest input, long timeout) {
		...
		Maquina.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Maquina.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Maquina.PagedResults> listMaquinaRequest(Maquina.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Maquina.PagedResults.class);
	}
	...
	* The 'list' request primitive for the Maquina entity.
	*/
	public CompletableFuture<Maquina.PagedResults> listMaquinaRequest(Maquina.PageRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Maquina.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
