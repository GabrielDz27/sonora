...
public class ListPecaImpl {
	...
	public ListPecaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Peca.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.LIST_PECA, //
	...
	*/
	public Peca.PagedResults listPeca(Peca.PageRequest input, long timeout) {
		...
		Peca.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Peca.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Peca.PagedResults> listPecaRequest(Peca.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Peca.PagedResults.class);
	}
	...
	* The 'list' request primitive for the Peca entity.
	*/
	public CompletableFuture<Peca.PagedResults> listPecaRequest(Peca.PageRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Peca.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
