 ...
 * 
 */
public class ListRegistroProducaoImpl {
	...
	public ListRegistroProducaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(RegistroProducao.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.LIST_REGISTRO_PRODUCAO, //
	...
	*/
	public RegistroProducao.PagedResults listRegistroProducao(RegistroProducao.PageRequest input, long timeout) {
		...
		RegistroProducao.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), RegistroProducao.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<RegistroProducao.PagedResults> listRegistroProducaoRequest(RegistroProducao.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, RegistroProducao.PagedResults.class);
	}
	...
	* The 'list' request primitive for the RegistroProducao entity.
	*/
	public CompletableFuture<RegistroProducao.PagedResults> listRegistroProducaoRequest(RegistroProducao.PageRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, RegistroProducao.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
