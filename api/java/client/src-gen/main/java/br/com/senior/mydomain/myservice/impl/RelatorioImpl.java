...
public class RelatorioImpl {
	...
	public RelatorioImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(RelatorioInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RELATORIO, //
	...
	*/
	public RelatorioOutput relatorio(RelatorioInput input, long timeout) {
		...
		RelatorioOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), RelatorioOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<RelatorioOutput> relatorioRequest(RelatorioInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, RelatorioOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<RelatorioOutput> relatorioRequest(RelatorioInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, RelatorioOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
