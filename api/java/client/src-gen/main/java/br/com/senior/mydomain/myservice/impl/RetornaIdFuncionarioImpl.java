 ...
 * 
 */
public class RetornaIdFuncionarioImpl {
	...
	public RetornaIdFuncionarioImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(RetornaIdFuncionarioInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETORNA_ID_FUNCIONARIO, //
	...
	*/
	public RetornaIdFuncionarioOutput retornaIdFuncionario(RetornaIdFuncionarioInput input, long timeout) {
		...
		RetornaIdFuncionarioOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), RetornaIdFuncionarioOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<RetornaIdFuncionarioOutput> retornaIdFuncionarioRequest(RetornaIdFuncionarioInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, RetornaIdFuncionarioOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<RetornaIdFuncionarioOutput> retornaIdFuncionarioRequest(RetornaIdFuncionarioInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, RetornaIdFuncionarioOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
