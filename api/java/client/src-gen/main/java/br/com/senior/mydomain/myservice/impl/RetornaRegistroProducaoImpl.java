 ...
 * 
 */
public class RetornaRegistroProducaoImpl {
	...
	public RetornaRegistroProducaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
	...
	private Message createMessage() {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				...
				MyServiceConstants.Commands.RETORNA_REGISTRO_PRODUCAO, //
				new byte[0]);
		}
	...
	*/
	public RetornaRegistroProducaoOutput retornaRegistroProducao(long timeout) {
		...
		RetornaRegistroProducaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), RetornaRegistroProducaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<RetornaRegistroProducaoOutput> retornaRegistroProducaoRequest() {
		...
		Message message = createMessage();
		addMessageHeaders(message);
		return serviceRunner.request(message, RetornaRegistroProducaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<RetornaRegistroProducaoOutput> retornaRegistroProducaoRequest(long timeout, TimeUnit unit) {
		...
		Message message = createMessage();
		addMessageHeaders(message);
		return serviceRunner.request(message, RetornaRegistroProducaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
