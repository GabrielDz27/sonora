 ...
 * 
 */
public class RetrieveMaquinaImpl {
	...
	public RetrieveMaquinaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Maquina.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETRIEVE_MAQUINA, //
	...
	*/
	public Maquina retrieveMaquina(Maquina.GetRequest input, long timeout) {
		...
		Maquina output = DtoJsonConverter.toDTO(resultMessage.getContent(), Maquina.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Maquina> retrieveMaquinaRequest(Maquina.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Maquina.class);
	}
	...
	* The 'retrieve' request primitive for the Maquina entity.
	*/
	public CompletableFuture<Maquina> retrieveMaquinaRequest(Maquina.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Maquina.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
