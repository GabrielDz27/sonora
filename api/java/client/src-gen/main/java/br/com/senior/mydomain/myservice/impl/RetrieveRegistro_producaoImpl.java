 ...
 * 
 */
public class RetrieveRegistro_producaoImpl {
	...
	public RetrieveRegistro_producaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Registro_producao.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETRIEVE_REGISTROPRODUCAO, //
	...
	*/
	public Registro_producao retrieveRegistro_producao(Registro_producao.GetRequest input, long timeout) {
		...
		Registro_producao output = DtoJsonConverter.toDTO(resultMessage.getContent(), Registro_producao.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Registro_producao> retrieveRegistro_producaoRequest(Registro_producao.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Registro_producao.class);
	}
	...
	* The 'retrieve' request primitive for the Registro_producao entity.
	*/
	public CompletableFuture<Registro_producao> retrieveRegistro_producaoRequest(Registro_producao.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Registro_producao.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
