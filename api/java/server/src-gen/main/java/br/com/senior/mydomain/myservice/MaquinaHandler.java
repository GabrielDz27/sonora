...
import static br.com.senior.mydomain.myservice.MaquinaEntity.SECURITY_RESOURCE;
...
import br.com.senior.mydomain.myservice.Maquina.PageRequest;
import br.com.senior.mydomain.myservice.Maquina.PagedResults;
import org.springframework.stereotype.Component;
import br.com.senior.platform.translationhub.api.TranslationHubApi;

@HandlerImpl(serviceClass = MyServiceConstants.class)
@Component("my_domain.my_service.MaquinaHandler")
public class MaquinaHandler implements CreateMaquina, CreateMergeMaquina, UpdateMaquina, DeleteMaquina, 
		 UpdateMergeMaquina, CreateBulkMaquina, RetrieveMaquina, ListMaquina, 
		 ImportMaquina, ExportMaquina {
	static final Logger logger = LoggerFactory.getLogger(MaquinaHandler.class);
	
	private static final String ID_FIELD_NAME = Arrays.stream(MaquinaEntity.class.getDeclaredFields())
	...
	@Inject
	MaquinaCrudService service;
	
	@Inject
	MaquinaDTOConverter dtoConverter;
	...
	public Maquina createMaquina(Maquina toCreate) {
		try {
			customValidator.validate(toCreate, entityDefProvider.getEntityDef("maquina"));
			MaquinaEntity entity = service.createMaquina(dtoConverter.toEntity(toCreate, true));
			...
			logger.error(e.getMessage(), e);
			throw new br.com.senior.messaging.model.ServiceException(br.com.senior.messaging.ErrorCategory.INTERNAL_ERROR, translationHubApi.getMessage("my_domain.my_service.maquina_illegalArgument", "An internal error occurred: "
				+ e.getClass().getSimpleName() + ". Please check the request parameters and try again."), e);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw new br.com.senior.messaging.model.ServiceException(br.com.senior.messaging.ErrorCategory.INTERNAL_ERROR, translationHubApi.getMessage("my_domain.my_service.maquina_internalError", "An internal error occurred: "
				+ e.getClass().getSimpleName() + ". Please check the request parameters and try again."), e);
		}
	}
	...
	@Transactional
	@Override
	public Maquina createMergeMaquina(Maquina toCreate) {
		try {
			MaquinaEntity entity = service.createMaquina(dtoConverter.toEntity(toCreate, false));
			return dtoConverter.toDTO(entity);
	...
	public Maquina updateMaquina(Maquina toUpdate) {
		try {
			customValidator.validate(toUpdate, entityDefProvider.getEntityDef("maquina"));
			MaquinaEntity modified = doUpdate(toUpdate);
	...
	@Transactional
	@Override
	public Maquina updateMergeMaquina(Maquina toUpdate) {
		try {
			customValidator.validate(toUpdate, entityDefProvider.getEntityDef("maquina"));
			MaquinaEntity entity = service.retrieveMaquina(new Maquina.Id(toUpdate.id));
			dtoConverter.mergeEntity(entity, toUpdate);
			entity = service.updateMaquina(entity);
			return dtoConverter.toDTO(entity);
	...
	private MaquinaEntity doUpdate(Maquina toUpdate) {
		java.util.Optional<MaquinaEntity> optionalEntity = service.getRepository().findById(java.util.UUID.fromString(toUpdate.id));
		
		if(optionalEntity.isEmpty())
			throw new br.com.senior.messaging.model.ServiceException(br.com.senior.messaging.ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getExternalMessage("my_domain", "my_service", "my_domain.my_service.maquina_notFound", "Maquina not found with id " + toUpdate.id));
		
		MaquinaEntity entity = optionalEntity.get();
		
		dtoConverter.updateEntity(entity, toUpdate);
		return doUpdate(entity);
	}
	
	private MaquinaEntity doUpdate(MaquinaEntity entity) {
		return service.updateMaquina(entity);
	}
	...
	public void deleteMaquina(Id id) {
		try {
			...
			throw new ServiceException(ErrorCategory.CONFLICT, translationHubApi.getMessage("my_domain.my_service.maquina_dataIntegrityViolation", "Data integrity violation to Delete Maquina. Error: " + ex.getMessage()), ex);
		}
	}
	...
	@Override
	public Maquina retrieveMaquina(Maquina.GetRequest request) {
		MaquinaEntity entity = service.retrieveMaquina(new Maquina.Id(request.id));
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listMaquina(PageRequest pageRequest) {
		...
		Page<MaquinaEntity> pages = this.service.listMaquinaPageable(skip, top, orderBy, filter);
		List<MaquinaEntity> entities = pages.getContent();
		List<Maquina> dtos = null;
	...
	@Transactional
	@Override
	public CreateBulkMaquinaOutput createBulkMaquina(CreateBulkMaquinaInput toCreate) {
		if (toCreate != null) {
			List<Maquina> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					customValidator.validate(bean, entityDefProvider.getEntityDef("maquina"));
					bean.validate();
				});
				
				List<MaquinaEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkMaquinaOutput();
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_CREATE)
	@Transactional
	@Override
	public ImportMaquinaOutput importMaquina(ImportMaquinaInput request) {
		...
		ImportMaquinaOutput output = new ImportMaquinaOutput(importer.getId());
		return output;
	}
	...
	@Transactional(readOnly = true)
	@Override
	public ExportMaquinaOutput exportMaquina(ExportMaquinaInput request) {
		/*
		 ...
		 */
		ExportMaquinaOutput output = new ExportMaquinaOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new MaquinaImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Maquina.class.getName());
					importer.setCrudService(MaquinaHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("maquina", "blob");
