...
public class MaquinaImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(MaquinaImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  MyServiceConstants.Events.IMPORT_MAQUINA_EVENT, new ImportMaquinaEventPayload(status));
		}
	}
