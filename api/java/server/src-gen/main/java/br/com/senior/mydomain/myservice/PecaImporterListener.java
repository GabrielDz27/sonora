...
public class PecaImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(PecaImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  MyServiceConstants.Events.IMPORT_PECA_EVENT, new ImportPecaEventPayload(status));
		}
	}
