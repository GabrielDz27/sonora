...
public class RegistroProducaoImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(RegistroProducaoImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  MyServiceConstants.Events.IMPORT_REGISTRO_PRODUCAO_EVENT, new ImportRegistroProducaoEventPayload(status));
		}
	}
