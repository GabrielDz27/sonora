...
public class Registro_producaoImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(Registro_producaoImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  MyServiceConstants.Events.IMPORT_REGISTROPRODUCAO_EVENT, new ImportRegistro_producaoEventPayload(status));
		}
	}
