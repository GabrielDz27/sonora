...
public class AdicaoImpl {
	...
	public AdicaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(AdicaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.ADICAO, //
	...
	*/
	public AdicaoOutput adicao(AdicaoInput input, long timeout) {
		...
		AdicaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), AdicaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<AdicaoOutput> adicaoRequest(AdicaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, AdicaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<AdicaoOutput> adicaoRequest(AdicaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, AdicaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
