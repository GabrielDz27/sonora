...
public class DivisaoImpl {
	...
	public DivisaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(DivisaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.DIVISAO, //
	...
	*/
	public DivisaoOutput divisao(DivisaoInput input, long timeout) {
		...
		DivisaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), DivisaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<DivisaoOutput> divisaoRequest(DivisaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, DivisaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<DivisaoOutput> divisaoRequest(DivisaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, DivisaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
