...
public class FindNomeByIdImpl {
	...
	public FindNomeByIdImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindNomeByIdInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_NOME_BY_ID, //
	...
	*/
	public FindNomeByIdOutput findNomeById(FindNomeByIdInput input, long timeout) {
		...
		FindNomeByIdOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindNomeByIdOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindNomeByIdOutput> findNomeByIdRequest(FindNomeByIdInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindNomeByIdOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<FindNomeByIdOutput> findNomeByIdRequest(FindNomeByIdInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindNomeByIdOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
