...
import br.com.senior.mydomain.myservice.MultiplicacaoInput;
import br.com.senior.mydomain.myservice.MultiplicacaoOutput;
 ...
 * 
 */
public class MultiplicacaoImpl {
	...
	public MultiplicacaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(MultiplicacaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.MULTIPLICACAO, //
	...
	*/
	public MultiplicacaoOutput multiplicacao(MultiplicacaoInput input, long timeout) {
		...
		MultiplicacaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), MultiplicacaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<MultiplicacaoOutput> multiplicacaoRequest(MultiplicacaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, MultiplicacaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<MultiplicacaoOutput> multiplicacaoRequest(MultiplicacaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, MultiplicacaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
