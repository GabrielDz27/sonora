...
public class SubtracaoImpl {
	...
	public SubtracaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(SubtracaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.SUBTRACAO, //
	...
	*/
	public SubtracaoOutput subtracao(SubtracaoInput input, long timeout) {
		...
		SubtracaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), SubtracaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<SubtracaoOutput> subtracaoRequest(SubtracaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, SubtracaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<SubtracaoOutput> subtracaoRequest(SubtracaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, SubtracaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
